...
import { Sparkles, Users, Coins, ChevronRight, ChevronLeft, Check, Lock, Unlock } from "lucide-react";
...
export function EpisodeCreator({ webtoonId, episodeNumber, credits, characters, projectBrief }: EpisodeCreatorProps) {
    ...
    <div className="space-y-6">
      ...
      {step === "cuts" && episodeId && storyJson && (
        <div className="space-y-4">
          <CutsPhase storyJson={storyJson} webtoonId={webtoonId} episodeId={episodeId} />
          <Button variant="outline" size="sm" onClick={() => router.push(`/studio/${webtoonId}`)}>
            <ChevronLeft className="h-4 w-4 mr-1" />대시보드로
          </Button>
        </div>
      )}
    </div>
